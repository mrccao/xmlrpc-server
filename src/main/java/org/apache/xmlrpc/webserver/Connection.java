...
public class Connection implements ThreadPool.InterruptableTask, ServerStreamConnection {
    ...
    private RequestData getRequestConfig() throws IOException {
        ...
        //增加额外信息 caochun 20191206
        requestData.setLocalAddr(socket.getLocalAddress().getHostAddress());
        requestData.setLocalName(socket.getLocalAddress().getHostName());
        requestData.setLocalPort(socket.getLocalPort());
        requestData.setRemoteAddr(socket.getInetAddress().getHostAddress());
        requestData.setRemoteHost(socket.getInetAddress().getHostName());
        requestData.setRemotePort(socket.getPort());
